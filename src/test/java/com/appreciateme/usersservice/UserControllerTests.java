...
public class UserControllerTests {
  ...
  void getUserById_GET_userExists_thenReturnStatusOk() throws Exception {
    ...
    Mockito.when(userService.existsById(id))
        .thenReturn(true);
  ...
  void createValidUser_POST_thenReturnStatusCreated() throws Exception {
    ...
    mockMvc.perform(
            post(endpoint).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(USER_1)))
        .andExpect(status().isCreated());
  }

  @Test
  void createInvalidUser_POST_thenReturnStatusCreated() throws Exception {
